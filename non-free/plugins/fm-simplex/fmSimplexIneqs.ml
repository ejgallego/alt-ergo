...
module Container : Inequalities.Container_SIG = struct
  module Make
    ...
    (P : Inequalities.EXTENDED_Polynome with type r = X.r)
            ...
            List.fold_left
              (fun dep (ld,(re_ld, eps_ld)) ->
                ...
                if Q.is_zero eps_ld then dep
                  ...
                  match MPL.bindings ineq.dep with
                    [a, (n,p, is_le)] ->
                      assert (Q.is_one n && is_le);
                      assert (not (MPL.mem a dep));
                      MPL.add a (eps_ld, p, is_le) dep
                  | _ -> assert false
              )MPL.empty vals
          in
          let ineq =
