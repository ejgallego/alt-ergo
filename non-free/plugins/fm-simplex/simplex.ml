  ...
  module Normalizer = struct
    ...
    let array_has_mem_sign ar =
      ...
      while !i < len && (!nb_pos = 0 || !nb_neg = 0) do
        ...
        if c < 0 then incr nb_neg;
        incr i;
      done;
      if !nb_neg <> 0 then if !nb_pos <> 0 then None else Some !nb_neg
      else if !nb_pos <> 0 then Some !nb_pos else Some 0
    ...
    let substs_from_equalities eqs h_i_s len =
      ...
      List.iter
        (fun p ->
           ...
           try
             match array_has_mem_sign p.a with
               | Some n ->
