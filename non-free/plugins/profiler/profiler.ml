...
module M : Profiling_default.S = struct
  ...
  type t = {
    ...
    t_elim : int ref;
    b_elim : int ref;
    (* first int: counter ok keeped instances,
  ...
  let new_instance_of axiom loc keeped =
    ...
    assert (ii.loc == loc);
    let ii =
      if keeped then{ii with keeped = ii.keeped + 1}
  ...
  let print_instances_generation forced steps timers =
    if not forced && !(state.instances_map_printed) then
      ...
      let insts =
        MS.fold
          (fun name ii acc ->
            let f1 = float_of_int ii.keeped in
      ...
      in
      let insts =
        List.fast_sort (fun (_,i1,r1) (_,i2,r2) ->
          (i1.keeped - i2.keeped) @@
      ...
      List.iter
        (fun (name, i, r) ->
          fprintf fmt "ratio keeped/all: %s | " (float_resize r 13);
          fprintf fmt "keeped: %s | " (int_resize i.keeped 7);
          fprintf fmt "ignored: %s | " (int_resize i.ignored 7) ;
          fprintf fmt "conflicted: %s | " (int_resize i.confl 7);
          fprintf fmt "consumed: %s | "
            (int_resize (T.Set.cardinal i.consumed) 6);
          fprintf fmt "produced: %s | "
            (int_resize (T.Set.cardinal i.produced) 6);
          fprintf fmt "new: %s || "
            (int_resize (T.Set.cardinal i._new) 6);
          fprintf fmt "%s | " (string_resize name 30);
          fprintf fmt "%a@." report3 i.loc;
        )insts;
      (*if forced then
...
(*
  ...
  let float_print fmt v =
    if v = 0. then fprintf fmt "--      "
    else if v < 10. then fprintf fmt "%0.6f" v
    else if v < 100. then fprintf fmt "%0.5f" v
    else fprintf fmt "%0.4f" v
  ...
  let line_of_sum_module timers =
    for i = 0 to 211 do fprintf fmt "-" done;
    fprintf fmt "|@.";
  ...
  let timers_table forced timers =
    ...
    List.iter
      (fun f ->
         fprintf fmt"| %s" (string_resize (Timers.string_of_ty_module f) 10))
      Timers.all_modules;
    fprintf fmt "|@.";
    for i = 0 to 211 do fprintf fmt "-" done;
    fprintf fmt "|@.";
...
let () = Profiling_default.set_current (module M : Profiling_default.S)
